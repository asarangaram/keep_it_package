...
class _MediaServicesState extends State<MediaServices> {
  ...
  Widget build(BuildContext context) {
    return switch ((widget.isPreview, widget.media.type)) {
      (false, _) => PreviewService(media: widget.media),
      (true, CLMediaType.image) => ImageViewService(
          file: File(widget.media.path),
          onLockPage: widget.onLockView,
        ),
      (true, CLMediaType.video) => VideoPlayerService.player(
          media: widget.media,
      ...
      (true, _) => throw UnimplementedError('Not yet implemented')
    };
  }
}
