...
class FetchThumbnailState extends ConsumerState<ImageThumbnail> {
  ...
  Widget build(BuildContext context) {
    ...
    final relativePath = CLMedia.relativePath(
      widget.media.path,
      pathPrefix: widget.directory,
      validate: false,
    );

    final uuid = uuidGenerator.v5(Uuid.NAMESPACE_URL, relativePath);
    final previewFileName = path.join(widget.directory, '$uuid.tn.jpeg');

    final hasThumbnail = !widget.refresh && File(previewFileName).existsSync();
    if (hasThumbnail) {
